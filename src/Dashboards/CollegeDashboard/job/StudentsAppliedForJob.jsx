...
const StudentsAppliedForJob = () => {
 ...
 // 2️⃣ when jobId changes, load its applicants
  ...
  <div className="mb-4">
  <label htmlFor="jobId" className="block mb-2 font-semibold text-gray-700">
    Select Job (Name of Job, Company Name, Closing Date):
  </label>
  <select
    id="jobId"
    value={jobId}
    onChange={handleJobChange}
    className="border border-gray-300 p-2 w-full rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white text-gray-700"
  >
    <option value="" disabled className="text-gray-500">Select a job</option>
    {jobs.map((job, index) => (
      <option key={job._id} value={job._id} className="text-xm text-gray-700 hover:bg-blue-100">
       {`${index + 1}. Job: ${job.title}, Company: ${job.company}, Closing: ${new Date(job.closingDate).toLocaleDateString()}`}
      </option>
    ))}
  </select>
</div>
